...
easypackages::libraries(c("vembedr", "knitr", "dplyr"))
source("R/utils.R")
...
3. Descrever as propriedades da água relevantes para `r small_caps("Hidrometeorologia")`
...
7. Definir bacia hidrográfica e explicar sua relevância para `r small_caps("Hidrometeorologia")`
...
```{r, include = FALSE}
#A atmosfera é a camada de ar em movimento que circunda a Terra. Em sua composição  predomina o Nitrogênio (N~2~) e o Oxigênio (O~2~) que juntos totalizam 99% do volume da atmosfera. Vários outros gases combinados quantificam menos de 1% do volume da atmosfera. Apesar disso, alguns desses gases exercem um importante papel no balanço de radiação terrestre através do efeito de estufa.

#A pressão atmosférica é uma medida da massa de ar acima de um dado ponto na superfície. A pressão total exercida por uma parcela de ar é a soma das pressões de todos os gases contidos na parcela. O N~2~, que compreende 78% do ar, exerce a maior pressão parcial, seguido do O~2~ (21%).  O vapor d'água tipicamente compreende de 1 a 4% do ar. Por exemplo a pressão atmosférica próxima ao nível é de $\approx$ 1000 hectopascals (hPa, 1 hPa = 100 Pa = 1 milibar). A pressão parcial do N~2~ é 780 hPa e do O~2~ é de 210 hPa. Se o vapor d'água compreende 1% da parcela sua pressão parcial é 10 hPa (ou 1000 Pa). Como o vapor d'água é um pequeno constituinte do ar a pressão de vapor é uma pequena componente da pressão total do ar. O CO~2~ tem uma pressão parcial de $\approx$ 40 Pa.

#Os principais gases estufa são o vapor d'água (H~2~O), o dióxido de carbono (CO~2). metano (CH~4~) e o Óxido Nitroso (N~2~O). A quantidade de vapor na atmosfera varia com a temperatura do ar.
```
...
```{block2, note-text-climate-system, type='rmdtip', echo = TRUE}
...
Uma forma útil de conceitualizar a Hidrometeorologia é através da representação simplificada do **ciclo hidrológico**, como mostrado na Figura \@ref(fig:cap2CicloHidro) e na Figura \@ref(fig:cap2CicloHidroAnim). O ciclo hidrológico é o movimento vertical e horizontal da água em suas diferentes fases (vapor, líquida ou sólida) entre a os oceanos, a atmosfera e a superfície terrestre `r if(knitr::opts_knit$get('rmarkdown.pandoc.to') != "latex"){ paste("(veja animação abaixo)")}`. Pode-se imaginar esse ciclo ocorrendo em diversas escalas espaciais (da local a global) temporais (minutos a décadas).


```{r cap2CicloHidro, fig.align='center', ref.label='cap2CicloHidro', fig.cap='Ilustração conceitual do Ciclo Hidrológico. Fluxos expressos em 10^3^ km^3^', out.width='95%'}
#if(output == "latex"){
 knitr::include_graphics('images/CH-ciclo-hidrol.png')
#}
```
...
```{r cap2CicloHidroAnim, fig.align='center', fig.cap='(ref:cap2CicloHidroAnim)', result = 'asis'}

#if(output != "latex"){
vembedr::embed_url("https://www.youtube.com/watch?v=Az2xdNu0ZRk") %>%
...
```{r cap2-ciclohidro-anim2, fig.align='center', ref.label='cap2-ciclohidro-anim2', fig.cap='Animação conceitual do ciclo hidrológico Fonte: asd.', results='asis', include = FALSE}
#![teste](images/basin_defined.swf)
#knitr::include_graphics('images/basin_defined.swf')
if (output != "latex") {
  swfhtml <- R2SWF::swf2html(
    swf.file = "images/2-hydro_cycle.swf", 
    output = FALSE
  )
  cat(
    paste(
...
#`r cat("Fonte: [COMET website](http://meted.ucar.edu/) UCAR, NOAA.[^5]")`

#[^5]: A fonte deste material é o Website COMET® http://meted.ucar.edu/ da *University Corporation for Atmospheric Research* (UCAR), patrocinado em parte através do acordo de cooperação com o *National Oceanic and Atmospheric Administration* (NOAA), U.S. *Department of Commerce* (DOC). ©1997-2017 *University Corporation for Atmospheric Research*. Todos direitos reservados.
```
...
```{r sealevel, fig.align='center', out.width='80%', fig.cap='(ref:sealevel)'}
  knitr::include_graphics("images/RiversAnimated_495013.gif")
```
...
```{r water-earth-data}
load("data/water_earth.rda")
water_earth[[1]] <- as.character(water_earth[[1]])
 #for (col in colnames(water_earth)){
  #Encoding(water_earth[[1]]) <- "'ISO-8859-1'"
  Encoding(water_earth[[1]]) <- "UTF-8"
  Encoding(names(water_earth)) <- "UTF-8"
#x <- water_earth[, 1]
#Encoding(x)
```

A `r small_caps("Hidrosfera")` envolve a água contida no planeta Terra armazenada em rios, lagos, oceanos, solo e ar. A Terra armazena $\approx$ `r round(sum(water_earth[-2, 2])/10^6)` milhões  de km^3^ de água (soma dos volumes de água doce e salgada da Tabela \@ref(tab:water-earth-table)). Comparado ao volume da Terra (1,08321 $\times$ 10^12^ km^3^), esse volume de água é $\approx$ `r gsub("\\.", ",", as.character(round((1377*10^6)/(1.08*10^12)*100, 2)))`% do volume do planeta (Figura \@ref(fig:wateravail)).
...
```{r wateravail, fig.align='center', out.width='100%', ref.label='wateravail', fig.cap='(ref:wateravail)'}
knitr::include_graphics('images/cook-EarthWater_88530.jpg')
```

O volume total de água espalhado sobre a área superficial da Terra ($\approx$ 510 milhões de km^2^) seria equivalente a uma lâmina de água de profundidade média de `r (1377*10^6)/(510*10^6)*1000` m.
...
```{r water-earth-table, encoding = "UTF-8"}
#load("data/water_earth.rda")
knitr::kable(
  water_earth,
...
Em média 12.700 km^3^ (`r round(12700/(510072000) * 1000 * 1000)` mm) de vapor d’água por ano existem na atmosfera. Para acomodar os 577.000 km^3^ de evaporação (veja seção \@ref(BHglobal)), o volume total de água contida na atmosfera deve ser reciclado (da atmosfera para superfície terrestre e oceano) mais de 40 vezes por ano ou $\approx$ `r round(12700/round((1+3.85)*119000) * 365.25)` dias. Isto é conhecido como tempo de residência ou de renovação (visto em mais detalhes na seção \@ref(balhid)).
...
```{r CH-estrutura-molec-agua, fig.align='center', out.width='100%', fig.cap='(ref:CH-estrutura-molec-agua)'}
knitr::include_graphics("images/CH-estrutura-molec-agua.jpg")
```
...
```{block2, CH-dica-diag-fases, type='rmdtip', echo = TRUE}
...
```{r CH-diag-fases-agua, fig.align='center', out.width='100%', fig.cap='(ref:CH-diag-fases-agua)'}
knitr::include_graphics("images/CH-diag-fases-agua3.jpg")
```
...
```{r CH-dens-agua-gelo, fig.align='center', out.width='90%', fig.cap='(ref:CH-dens-agua-gelo)'}
knitr::include_graphics("images/Density_of_ice_and_water_(en).svg")
```
...
```{r pqgeloflutua, fig.align='center', fig.cap='(ref:pqgeloflutua)', result = 'asis'}
vembedr::embed_url("https://www.youtube.com/watch?v=UukRgqzk-KE") %>%
  htmltools::div(align = "center")
knitr::include_graphics('images/empty.png')
```
...
A `r small_caps("Evaporação")` é processo físico pelo qual a água líquida nos oceanos ou no continente muda para vapor no ar. Isto ocorre quando o ar não saturado entra em contato com uma superfície úmida. A evaporação fornece umidade para atmosfera que retorna para a superfície como chuva ou neve. A `r small_caps("Evaporação")` também consome uma grande quantidade de calor, o que ajuda a resfriar a superfície evaporante. 
...
O ponto inicial da maioria das análises hidrometoerológicas envolvem a aplicação do `r small_caps("balanço de massa")`. A equação do `r small_caps("balanço de massa")` para um dado sistema pode ser derivada a partir do `r small_caps("teorema do Transporte de Reynolds")`  [@Mays2005]. 


```{r}
# boa ref com exemplos e exercícios (água precipitável)
#http://www.engr.colostate.edu/~ramirez/ce_old/classes/cive322-Ramirez/CIVE322-Basic-Hydrology.htm
```


O teorema inicia com a definição de um `r small_caps("volume de controle")` no espaço, definido por uma `r small_caps("superfície  de controle")` (Figura \@ref(fig:volcontrol)).

(ref:volcontrol) Esquema básico de um `r small_caps("volume de controle")` com fluxos através da superfície de controle.

```{r volcontrol, fig.align='center', out.width='100%', ref.label='volcontrol', fig.cap='(ref:volcontrol)'}
knitr::include_graphics('images/2-volume-controle-pt-br.png')
```
...
```{block2, tempo-residencia, type='rmdnote', echo = TRUE}
...
```{r prep-water-earth-table2, echo = FALSE}
prec_trenb <- (113 + 373)*10^3 # km3/ano
area_terra <- 510*10^6 
#prec_trenb/area_terra * 10^6

water_earth2 <- slice(water_earth, -2) %>%
  select(-(3:4))

flux <- round(prec_trenb/365) #1415
tempos_resid_an <- water_earth2[[2]]/(flux*365)
#format(tempos_resid, digits = 10, nsmall = 1)
tempos_resid_an[2] <- NA
tempos_resid_an[4:6] <- tempos_resid_an[4:6] * 365

water_earth2 <- water_earth2 %>%
  mutate(
  `Tempo de residência` = round(tempos_resid_an),#c("2500 anos", "-", "8 anos", "88 dias", "47 dias", "9 dias")
  unidade = c(rep("anos", 3), rep("dias", 3))
)
```


```{r water-earth-table2, encoding = "UTF-8"}
#load("data/water_earth.rda")
knitr::kable(
  water_earth2,
  caption = "Estimativas dos tempo de residência nos principais reservatórios hidrológicos globais. Estes valores assumem uma taxa média global de fluxo hidrológico de 1332 km^3^ dia^-1^. Adaptado de @Trenberth2007.",
  booktabs = TRUE,
  row.names = FALSE,
  # align = "r",
  format.args = list(
...
```{r balan-global, fig.align='center', out.width='70%', fig.cap='(ref:balan-global)'}
knitr::include_graphics('images/2-balanco-global.png')
```

```{r, include = FALSE}
#Um aspecto importante do cilo hidrológico é que em escala global o sistema é essencialmente fechado com respeito a água. Uma quantidade desprezível está entrando ou saindo do sistema. Em outras palavras, não há forçante externa em termos de fluxo de água e nestes casos,  o sistema geralmente tende a atingir um estado de equilíbrio.
```

Anualmente, $\approx$ `r sprintf("%i", round((1+3.85)*119000))` km^3^ ano^-1^ de água (o que equivale[^6] a `r round((1+3.85)*119000/(510072000) * 1000 * 1000)` mm), deixam a atmosfera na forma de `r small_caps("Precipitação")`. A mesma quantidade retorna para atmosfera anualmente como `r small_caps("Evaporação")`, por isso a quantidade de água na atmosfera não muda com o tempo e o ciclo hidrológico na escala global é fechado.
...
A água não é uniformemente distribuída entre os continentes e o oceano. A `r small_caps("Precipitação")` média nos continente é 119000 km^3^ (cerca de `r  round(119000/577150 * 100)`% da `r small_caps("Precipitação")` média global. Tomando este valor como unidade relativa (u.r) para os demais fluxos do balanço hídrico global (100 u.r = 119.000 km^3^), 61% desta precipitação (`r round((0.61)*119000/(148940000) * 1000 * 1000)` mm) evapora de volta para atmosfera. 

Nos oceanos a evaporação supera a precipitação, resultando em uma diferença de -39 u.r. Este excedente de evaporação dos oceanos ($\approx$ `r round(39/424 * 100)`%) é transportado pelo vento para os continente e contribui para  precipitação nos continentes[^9]. No continente a precipitação excede a evapotranspiração gerando um excesso (39 u.r, ou `r round((0.39)*119000/(148940000) * 1000 * 1000)` mm) que escoa em grande parte pela superfície e o subsolo para o oceano, reestabelecendo a maior perda de água por evaporação no oceano. 

[^9]: Se esta fosse a única fonte de precipitação a média global da precipitação seria somente cerca de `r round(119000/(510072000) * 1000 * 1000)` mm, um valor típico de regiões semi-áridas e desertos.
...
```{r clima-agua-precip, fig.align='center', out.width='80%', fig.cap='(ref:clima-agua-precip)'}
if(output == "latex"){
 knitr::include_graphics('images/pwat_climatology_nasa_randel1996.png')
} else {
  knitr::include_graphics('images/pwat_web_climvis.gif')
}
...
```{r div-hor-vapor, fig.align='center', out.width='100%', fig.cap='(ref:div-hor-vapor)'}
knitr::include_graphics('images/DivFluxVaporAnual-Peixoto.jpg')
```
...
`r small_caps("Bacia Hidrográfica")` (BH).

A BH é limitada ao longo de sua borda por pontos mais altos do relevo, chamados `r small_caps("Dividores de Água")` – morros, serras ou montanhas; abaixo por rocha e acima pela atmosfera. Uma `r small_caps("Bacia Hidrográfica")`, em geral, possui muitas nascentes, mas uma saída única, o rio principal[^8], responsável pelo escoamento da água captada.

[^8]: Rio de maior extensão na rede de drenagem da BH.

Uma gota de água no lado da encosta direcionada para o rio escoa na direção do rio. A gota de água no outro lado do divisor escoa para outro rio `r if(knitr::opts_knit$get('rmarkdown.pandoc.to') != "latex"){ paste("(veja animação abaixo)")}`.


```{r animabacia, fig.align='center', out.width='100%', ref.label='animabacia', fig.cap='Animação da bacia. Fonte: asd.', results='asis'}
# ![teste](images/basin_defined.swf)
# knitr::include_graphics('images/basin_defined.swf')
#if (output != "latex") {
  swfhtml <- R2SWF::swf2html(
    swf.file = "images/2-basin_defined.swf",
    output = FALSE
  )
  cat(paste("<div align = \"%s\">\n", swfhtml, "\n</div>"))
  cat("Fonte: [COMET website](http://meted.ucar.edu/) UCAR, NOAA.")
#}
...
```{block2, definicao-bh, type='rmdnote', echo = TRUE}
...
```{r secao-transv, fig.align='center', out.width='60%', ref.label='secao-transv', fig.cap='Seção transversal de rio.'}
knitr::include_graphics('images/secao-transversal.png')
```

A localização da seção transversal (Figura \@ref(fig:secao-transv)) do ponto exutório é determinada pela proposta da análise. Em geral, as BH são delineadas a partir de estações fluviométricas (onde a vazão é medida) ou acima de uma junção com um rio maior, um lago, um reservatório, uma usina hidrelétrica ou um oceano. A região localizada acima deste ponto, ou seja, no sentido contra a corrente do rio, é chamada `r small_caps("Montante")`. A região abaixo deste ponto, no sentido da corrente do rio, é denominada `r small_caps("Jusante")`. 

Existe um número infinito de pontos ao longo do rio, então é possível definir inúmeras BH para um rio específico (Figura \@ref(fig:nested-basins)). A água escoa morro abaixo até um arroio mais próximo, onde aquela água escoará para um rio à jusante através da `r small_caps("Rede de Drenagem")`. Rios tributários menores deságuam em outros rios maiores que em última instância vão desaguar no oceano. A `r small_caps("Rede de Drenagem")` é a organização dos padrões de drenagem sobre a paisagem da BH. BHs de montante são aninhadas à BHs de jusante e são chamadas sub-bacias hidrográficas. Esta hierarquia pode ter diferentes níveis de acordo com a BH. Em geral, são definidos pelo menos 3 níveis, de acordo com a escala espacial: `r small_caps("Bacias Hidrográficas")`, `r small_caps("Sub-bacias Hirográficas")` e `r small_caps("Micro-Bacias Hidrográficas")`.


(ref:nested-basins) Hierarquia de Bacias Hidrográficas aninhadas. Fonte: @Marsh1987.

```{r nested-basins, fig.align='center', out.width='50%', ref.label='nested-basins', fig.cap='(ref:nested-basins)'}
knitr::include_graphics('images/2-nested-hierarchy-basins-crop.png')
```

A projeção horizontal da área da BH é chamada `r small_caps("Área de Drenagem")` e inclui toda superfície à montante do exutório. Embora as divisões políticas, em geral, não sigam os limites das BH, agências de planejamento dos recursos hídricos e do uso da terra reconhecem que a gestão efetiva da qualidade e quantidade da água requer a perspectiva da BH.
...
```{r bacia-efetiva, fig.align='center', out.width='80%', ref.label='bacia-efetiva', fig.cap='Distinção entre BH topográfica e BH efetiva. Fonte: Adaptado de http://echo2.epfl.ch/e-drologie/chapitres/chapitre2/chapitre2.html.'}
knitr::include_graphics('images/2-bacia-efetiva-pt-br.png')
```
...
```{r bacia-modif, fig.align='center', out.width='90%', ref.label='bacia-modif', fig.cap='(ref:bacia-modif)'}
knitr::include_graphics('images/bacia-modificada.png')
```
...
```{r animacao}
knitr::include_graphics("images/all_frames.gif")
```


```{r, eval = FALSE, include = FALSE}
library(raster)
laea <- projection(shapefile("~/Dropbox/DBHM/data_process/geo_data/longestPath_mogu.shp"))
ll <- projection(readRDS("~/Dropbox/DBHM/data_process/geo_data/data_base/br_states.rds"))
 
 mdet <- raster("~/Dropbox/DBHM/data_process/geo_data/softs/Paz2006_routines/MNTAlta/vacacai/MNTfill.rst")
 projection(mdet) <- laea
 mdet_ll <- projectRaster(mdet, crs = ll, res = res(mdet)[1]/(100*10^3))
 plot(mdet_ll)
 plot(mdet_ll); cab <- drawLine(sp = TRUE)
(cab <- purrr::flatten(coordinates(cab))[[1]][1,])
 
 fd <- terrain(mdet_ll, "flowdir")
 plot(fd)
 path <- flowPath(fd, p = cab)
...
```{r}
# Incluir Bacias endorréicas e exorréicas

#Pathways generating river flow
# Livro Hydrology in Practice
...
if (output != "latex") {
  swfhtml <- R2SWF::swf2html(
    swf.file = "images/infilt_runoff.swf",
    output = FALSE
  )
  cat(paste("<div align = \"%s\">\n", swfhtml, "\n</div>"))
}
...
if (output != "latex") {
  swfhtml <- R2SWF::swf2html(
    swf.file = "images/infilt_runoff_generic.swf",
    output = FALSE
  )
  cat(paste("<div align = \"%s\">\n", swfhtml, "\n</div>"))
}
...
if (output != "latex") {
  swfhtml <- R2SWF::swf2html(
    swf.file = "images/infilt_runoff_hvyrain.swf",
    output = FALSE
  )
  cat(paste("<div align = \"%s\">\n", swfhtml, "\n</div>"))
}
...
if (output != "latex") {
  swfhtml <- R2SWF::swf2html(
    swf.file = "images/infilt_runoff_ltrain.swf",
    output = FALSE
  )
  cat(paste("<div align = \"%s\">\n", swfhtml, "\n</div>"))
}
...
```{r , fig.align='center', out.width='50%',  results='asis'}
if (output != "latex") {
  swfhtml <- R2SWF::swf2html(
    swf.file = "images/infilt_excess_flow.swf",
    output = FALSE
  )
  cat(paste("<div align = \"%s\">\n", swfhtml, "\n</div>"))
}
...
if (output != "latex") {
  swfhtml <- R2SWF::swf2html(
    swf.file = "images/satur_excess_flow.swf",
    output = FALSE
  )
  cat(paste("<div align = \"%s\">\n", swfhtml, "\n</div>"))
}
...
```{r, include = FALSE}
#Os fluxos de água na superfície e pouco abaixo dela são ilustrados esquematicamente na Figura 1.5. Eventos de precipitação trazem água para a superfície do solo, e de lá a água pode fazer uma dentre três coisas. Ela pode se infiltrar no solo e se tornar infiltração, pode fluir pela superfície do solo como escoamento superficial ou pode se evaporar da superfície depois que a precipitação termina. A água na zona insaturada que se move para baixo e flui para a zona saturada é chamada de recarga.
```
...
if (output != "latex") {
  swfhtml <- R2SWF::swf2html(
    swf.file = "images/infiltration_runoff.swf",
    output = FALSE
  )
  cat(paste("<div align = \"%s\">\n", swfhtml, "\n</div>"))
}
...
if (output != "latex") {
  swfhtml <- R2SWF::swf2html(
    swf.file = "images/surface_cover.swf",
    output = FALSE
  )
  cat(paste("<div align = \"%s\">\n", swfhtml, "\n</div>"))
}
...
```{r, include = FALSE}
# alternativas para inserir para swf
#<embed src="images/basin_defined.swf" quality="high" pluginspage="http://www.macromedia.com/go/getfashplayer" type="application/x-shockwave-flash" width="690" height="430">

#<object width="690" height="430">
#    <param name="movie" value="images/basin_defined.swf">
#    <embed src="images/basin_defined.swf" width="690" height="430">
#    </embed>
#</object>
```

```{r, include = FALSE}
cmd <- "locate *coverage*.swf"
dest_dir <- "./images"
files <- system(cmd, intern = TRUE)
purrr::walk(files, function(ifile){
  # ifile = files[1]
  file.copy(from = ifile, to = file.path(dest_dir, basename(ifile)), overwrite = TRUE)  
})
```
